...
export default function AppHome(){
  ...
  async function generateQuestions(){
    setLoading(true)
    try{
      const data = await generatePackage(goal) // { prompt, type, questions }
      ...
      <section className="max-w-7xl mx-auto px-4 lg:px-8 py-12 grid lg:grid-cols-2 gap-8">
        <div className="glass rounded-3xl p-6">
          ...
          <textarea
            ...
            placeholder="e.g., Write a persuasive email inviting customers to try our new product…"
          ...
          >
            {loading? 'Thinking…':'Generate Follow-up Questions'}
          </button>

          {taskType && (
            <div className="mt-3 text-xs text-white/60">
              Detected task: <span className="text-white/80">{taskType}</span>
            </div>
          )}
        </div>
